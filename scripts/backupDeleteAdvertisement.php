...
if ($delete)
{
	$response = mysql_query("SELECT owner_code FROM advertisement WHERE adid='$adid'");
	if ($response)
	{
	...
	if ($code == $owner_code)
	{
		if (!mysql_query("DELETE FROM advertisement WHERE adid = '$adid';")) echo "<script>alert('Something went wrong, please try again later.');</script>";
