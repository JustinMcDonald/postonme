...
isset($_POST['all']) ? $all = true : $all = false;
...
$query = "SELECT id, adid, poster FROM conversation WHERE inquirer='".mysql_real_escape_string($user) . "'";
if (!$all) $query .= " AND active_inquirer=true";
...
if ($_SESSION['online'])
{
	$query = "SELECT id, adid, inquirer FROM conversation WHERE poster='".mysql_real_escape_string($user)."'";
	if (!$all) $query .= " AND active_poster=true";
	$conversations = mysql_query($query);
	while($conversation = mysql_fetch_array($conversations))
	{
		$conversation_id = $conversation['id'];
