...
$query = "SELECT id, adid, poster FROM conversation WHERE inquirer='$user' AND active_inquirer=true" . $constraints;
...
if ($_SESSION['online'])
{
	$query = "SELECT id, adid, inquirer FROM conversation WHERE poster='$user' AND active_poster=true".$constraints;
	$conversations = mysql_query($query);
	while($conversation = mysql_fetch_array($conversations))
	{
		$conversation_id = $conversation['id'];
